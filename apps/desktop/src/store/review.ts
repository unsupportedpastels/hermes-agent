...
// Exact mounted composer surface that opened the pane. Null means the origin
// is unavailable (for example after a reload) and agent-ship must fail closed.
export const $reviewScopeSurfaceId = atom<null | string>(null)
...
export function openReview(
  ...
  scopeTarget = 'main',
  scopeSurfaceId: null | string = null
): void {
  $reviewScopeCwd.set(scopeCwd?.trim() || null)
  $reviewScopeTarget.set(scopeTarget.trim() || 'main')
  $reviewScopeSurfaceId.set(scopeSurfaceId?.trim() || null)
  $reviewOpen.set(true)
  void refreshReview()
  void refreshShipInfo()
}
...
export function toggleReview(
  ...
  if (matchesQuery(SIDEBAR_COLLAPSE_MEDIA_QUERY)) {
    ...
    const surfaceId = scopeSurfaceId?.trim() || null

    const originChanged =
      ($reviewScopeCwd.get() ?? null) !== (scopeCwd?.trim() || null) ||
      $reviewScopeTarget.get() !== target ||
      $reviewScopeSurfaceId.get() !== surfaceId

    if (!$reviewOpen.get() || originChanged) {
      openReview(scopeCwd, target, surfaceId)
    }
...
export function revealReview(
  ...
  if (!wasOpen) {
    ...
    $reviewScopeSurfaceId.set(surfaceId)
  }
