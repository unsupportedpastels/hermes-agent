...
describe('useComposerSubmit external request routing', () => {
  ...
  it('submits only through the visible composer whose target matches', async () => {
    const visibleMain = renderSubmitHook({ sessionKey: 'main-session' })
    const hiddenMain = renderSubmitHook({ sessionKey: 'hidden-session', visible: false })
  ...
  it('routes a tile-targeted submit to that tile only', async () => {
    ...
    requestComposerSubmit('ship project B', { target: 'tile:project-b' })
